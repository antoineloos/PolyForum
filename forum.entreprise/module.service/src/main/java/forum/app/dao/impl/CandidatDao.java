...
public class CandidatDao {
    ...
    public int connecter(String login, String pwd) throws Exception {
        ...
        if (listeCandidat.stream().anyMatch((ca) -> ca.getPassword().equals(pwdH))) {
            // if (c != null && listeCandidat.stream().anyMatch<Candidat>(c->c.getPassword().equals(pwd)) ){
            return ((Candidat) listeCandidat.stream().filter((c) -> c.getPassword().equals(pwdH)).toArray()[0]).getIdCandidat();
        }
        // Code erreur
        return -1;
    }
