...
(function() {
	...
	function getDurationHours(startHour, endHour) {
		return endHour >= startHour ? endHour - startHour : 24 + endHour
				- startHour;
	}
	...
	Timetable.Renderer.prototype = {
		draw : function(selector) {
			...
			function appendTimetableAside(container) {
				var asideNode = container.appendChild(document
						.createElement('aside'));
				var asideULNode = asideNode.appendChild(document
						.createElement('ul'));
			...
			function appendRowHeaders(ulNode) {
				for (var k = 0; k < timetable.locations.length; k++) {
					...
					var spanNode = liNode.appendChild(document
							.createElement('span'));
			...
			function appendTimetableSection(container) {
				var sectionNode = container.appendChild(document
						.createElement('section'));
				var timeNode = sectionNode.appendChild(document
						.createElement('time'));
			...
			function appendColumnHeaders(node) {
				...
				var headerNode = node.appendChild(elem);
				...
				for (var hour = timetable.scope.hourStart; !completed;) {
					var liNode = headerULNode.appendChild(document
							.createElement('li'));
					var elem = document.createElement('span');
					elem.setAttribute('id', i);
					var spanNode = liNode.appendChild(elem);
					spanNode.className = 'time-label';
					spanNode.textContent = prettyFormatHour(hour);

					if (hour === timetable.scope.hourEnd
			...
			function appendTimeRows(node) {
				...
				for (var k = 0; k < timetable.locations.length; k++) {
					...
					var idElem = timetable.locations[k].replace(/ /g,"_");
					idElem = idElem.replace('\(',"").replace('\)',"")
					element.setAttribute("id", idElem);
					element.setAttribute("data-question-id", k);
					element.className = 'droppable time-line';
					var liNode = ulNode.appendChild(element);
					appendLocationEvents(timetable.locations[k], liNode, k);/**/
				}
			}
			function appendLocationEvents(location, node, j) {
				for (var k = 0; k < timetable.events.length; k++) {
					var event = timetable.events[k];
					if (event.location === location) {
						appendEvent(event, node, location, j);
					}
				}
			}
			function appendEvent(event, node, location, k) {
				...
				if (hasOptions) {
					...
					hasAdditionalClass = (event.options.class !== undefined) ? true
							: false;
					hasDataAttributes = (event.options.data !== undefined) ? true
							: false;
				}

				var elementType = hasURL ? 'a' : 'span';
				var element = document.createElement(elementType);
				var res = event.name.split(" ");
				var idElem  = res[0] + " " + res[1];
				idElem = idElem + " " + location.replace('\(',"").replace('\)',"");
				element.setAttribute("id", idElem.replace(/ /g,"_"));
				element.setAttribute("data-question-id", k);
				var aNode = node.appendChild(element);
				var smallNode = aNode.appendChild(document
						.createElement('small'));
				...
				if (hasDataAttributes) {
					for ( var key in event.options.data) {
						aNode.setAttribute('data-' + key,
								event.options.data[key]);
					}
				}

				aNode.className = hasAdditionalClass ? 'time-entry draggable '
						+ event.options.class : 'time-entry draggable';
			...
			function computeEventBlockOffset(event) {
				...
				var eventStartHours = event.startDate.getHours()
						+ (event.startDate.getMinutes() / 60);
				var hoursBeforeEvent = getDurationHours(scopeStartHours,
						eventStartHours);
			...
			var scopeDurationHours = getDurationHours(
					timetable.scope.hourStart, timetable.scope.hourEnd);
